...
void server_connect() {
    ...
    msgs.push_back("[INF] Инициализация");
...
void v_chat(HWND& hwnd, PaStream *stream) {
    ...
    if (log_inf == false) {
        msgs.push_back("[INF] Инициализация окна");
        log_inf = true;
    }
    ...
    if (!voice_active) {
        ...
        if (c == 1) {
            if (ImGui::Button("Connect", ImVec2(x * 0.06, y * 0.05))) {
                ...
                msgs.push_back("[INF] Подключение к войсу");
    ...
    if (voice_active == true) {
        ...
        if (btn == true) {
            ...
            if (Pa_IsStreamActive(stream) == 0) {
                std::thread audio_thread([&]() {
                    ...
                    msgs.push_back("[INF] Включение микрофона");
                    });
                audio_thread.detach();
            }
            ...
            if (Pa_IsStreamActive(stream) == 1) {
                msgs.push_back("[INF] Выключение микрофона");
        ...
        if (ImGui::Button(ICON_FA_PHONE, ImVec2(x * 0.03, y * 0.05))) {
            if (Pa_IsStreamActive(stream) == 1) {
                ...
                msgs.push_back("[INF] Выход с войса");
            }
            voice_active = false;
    ...
    if (log_inf2 == false) {
        msgs.push_back("[INF] Инициализация окна 2");
        log_inf2 = true;
    }
    ...
    if (ImGui::InputText(" ", buf, 1024, ImGuiInputTextFlags_EnterReturnsTrue)) {
        if (std::string(buf) == "" || std::string(buf) == " ") {
            ...
            msgs.push_back("[INF] Отправлено сообщение: " + std::string(buf));
            buf[0] = '\0'; 
            autoScroll = true;
...
void console(ImGuiIO& io) {
    ...
    if (log_inf3 == false) {
        msgs.push_back("[INF] Запущена консоль");
        log_inf3 = true;
    }
    ...
    if (ImGui::InputText("Enter ur command", buffer, 512, ImGuiInputTextFlags_EnterReturnsTrue)) {
        msgs.push_back("[INF] Обработка команды: " + std::string(buffer));
...
int main(int, char**)
{
    ...
    msgs.push_back("[INF] Стили загружены");
